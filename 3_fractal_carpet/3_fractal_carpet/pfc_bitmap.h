...
#define PFC_BITMAP_VERSION_MAJOR      2         /* gro�e �nderungen           */
...
namespace pfc
{
	...
	template <typename T>
	inline bool is_lt(T const& a, T const& b)
	{
		return a < b;
	}

	template <typename T>
	inline bool is_eq(T const& a, T const& b)
	{
	...
	inline bool is_zero(T const& a)
	{
		...
		return pfc::is_eq(a, zero); // a == zero
	}
	...
	{
		static T const zero = T();
		return pfc::is_negative(a) ? zero - a : a;
	}

	template <typename T>
	inline T ceil_div(T const a, T const b)
	{
	...
	template <typename T>
	inline T multiple_of(T const a, T const b)
	{
		PFC_STATIC_ASSERT(std::is_integral <T>::value);
	...
	template <typename T>
	inline bool cstr_empty(T* const p_cstr)
	{
		return (p_cstr == nullptr) || (*p_cstr == T(0));
	}

	template <typename T>
	inline T* mem_copy_ptr(T* const p_dst, T const* const p_src, int const size)
	{
	...
	template <typename T>
	inline T* mem_set_ptr(T* const p_dst, int const c, int const size)
	{
	...
	template <typename T>
	inline T& mem_copy(T& dst, T const& src)
	{
		pfc::mem_copy_ptr(&dst, &src, 1);
		return dst;
	}

	template <typename T>
	inline T& mem_set(T& dst, int const c = 0)
	{
		pfc::mem_set_ptr(&dst, c, 1);
		return dst;
	}

	template <typename T>
	inline T& mem_reset(T& dst)
	{
		return pfc::mem_set(dst);
	}

	template <typename T>
	inline std::istream& read_ptr(std::istream& in, T* const ptr, int const count = 1)
	{
	...
	template <typename T>
	inline std::istream& read(std::istream& in, T& val)
	{
		return pfc::read_ptr(in, &val, 1);
	}

	template <typename T>
	inline std::ostream& write_ptr(std::ostream& out, T const* const ptr, int const count = 1)
	{
	...
	template <typename T>
	inline std::ostream& write(std::ostream& out, T const& val)
	{
...
#pragma pack (pop)
	...
	public:
		...
		void create(int const width, int const height, pfc::byte_t* p_image = nullptr)
		{
			...
			m_file_header.size = static_cast<pfc::dword_t>(sizeof(file_header_t) + sizeof(info_header_t) +
				get_size_in_file(width, height));
			...
			pfc::mem_set_ptr(
				(m_p_image = (p_image == nullptr)
					             ? pointer_pair<pfc::byte_t>(allocate<pfc::byte_t>(get_size(width, height)))
		...
		bool from_file(char const* const p_filename)
		{
			...
			{
				std::ifstream in(p_filename, std::ios_base::binary);
				ok = from_stream(in);
			}
		...
		bool from_stream(std::istream& in)
		{
			...
			if (in.good())
			{
				...
				if (is_valid(m_file_header) && is_valid(m_info_header))
				{
					...
					{
						pfc::read_ptr(in, line.data(), static_cast<int>(line.size()));
						auto const* p_src(line.data());
		...
		bool to_file(char const* const p_filename) const
		{
			...
			if (!pfc::cstr_empty(p_filename))
			{
				std::ofstream out(p_filename, std::ios_base::binary);
				ok = to_stream(out);
			}
	...
	private:
		...
		public:
			...
			pointer_pair(pointer_pair&& src) : m_ptr(src.m_ptr), m_uptr(std::move(src.m_uptr))
			{
				// move construction
				src.m_ptr = nullptr;
			}
			...
			pointer_pair& operator =(pointer_pair&& rhs)
			{
				// move assignment
				if (&rhs != this)
				{
					m_ptr = rhs.m_ptr;
					m_uptr = std::move(rhs.m_uptr);
					rhs.m_ptr = nullptr;
				}
		...
		template <typename T>
		static std::unique_ptr<pfc::byte_t[]> allocate(int const size)
		{
		...
		void copy(bitmap const& src)
		{
			if (&src != this)
			{
				...
				pfc::mem_copy_ptr(
					(m_p_image = pointer_pair<pfc::byte_t>(allocate<pfc::byte_t>(src.get_size()))).get(), src.m_p_image.get(),
					src.get_size()
				);
			}
		}
